/* eslint-disable */
/**
 * Vendored from React Bits — https://reactbits.dev  (MIT)
 * Source: src/ts-tailwind/Backgrounds/WebThreads/WebThreads.tsx
 *
 * Copied rather than installed because React Bits is a copy-in library, not a
 * runtime dependency. Kept as close to upstream as possible so it can be
 * re-synced: the only edits are the ones Atlas's `strict` TypeScript requires
 * (non-null assertions on regex groups and shader uniforms, which are provably
 * present, and a guarded IntersectionObserver entry). Tuning lives at the call
 * site in `../HomeBackdrop.tsx`, never in here.
 */
